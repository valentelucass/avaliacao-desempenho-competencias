...
public final class EvaluationCycleReadAuthorizationPolicy {
  ...
  public Optional<EvaluationCycleReadScope> scopeFor(EvaluationCycleReadAccessContext actor) {
    ...
    if (access.has(CYCLES_MANAGE) || access.has(INDICATORS_VIEW)) {
      return Optional.of(new EvaluationCycleReadScope(true, false, false, false));
    }

    boolean managedCollaborators = access.has(EVALUATE_LINKED);
    boolean directorManagedCollaborators = access.has(EVALUATE_LINKED_MANAGERS);
    boolean ownCollaborator = access.has(FILL_OWN_SELF_ASSESSMENT);
    if (!managedCollaborators && !directorManagedCollaborators && !ownCollaborator) {
      return Optional.empty();
    }
    return Optional.of(
        new EvaluationCycleReadScope(
            false, managedCollaborators, directorManagedCollaborators, ownCollaborator));
  }
}
