...
| Status                  | Regras iniciais complementadas pela [Regra operacional 2024.1](regras-operacionais-v1.md). Em 2026-08-29, a fonte implementa autenticação, autorização, exportação agregada e auditoria; `AVALIACAO_DEV` e `AVALIACAO_PROD` reconciliaram `V0001`–`V0013`; o cenário autenticado fictício passou em DEV; e o release público respondeu com os controles HTTP esperados. Isso não equivale a aceite de negócio ou uso de dados reais. |
...
Continuam fora desta decisão a criação de pessoas reais, o provisionamento de novas credenciais/segredos, a política formal de retenção de logs e backups e qualquer descarte futuro. Sessão e auditoria passaram no cenário autenticado automatizado de DEV; navegador e tecnologia assistiva manuais permanecem condições externas de aceite.
