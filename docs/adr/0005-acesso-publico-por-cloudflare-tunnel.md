...
Os hosts públicos do front-end e de CSRF da API responderam em leitura anônima, com os dois processos PM2 do projeto online na evidência registrada. O cenário autenticado passou somente no runtime local contra DEV. Essa observação não certifica o Cloudflare: redirecionamento HTTP→HTTPS, WAF/bot protection, limite de borda, revalidação dos headers finais e teste após reinicialização continuam externos, assim como backup/restauração e aceite de negócio.
