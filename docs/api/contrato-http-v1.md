...
Correção de 16/09/2026 (`ADC-COR-019`): a inicialização da SPA cancela a continuação de efeitos encerrados, inclusive na execução duplicada do StrictMode em desenvolvimento. Chamadas simultâneas de renovação explícita e recuperação de 401 compartilham uma única requisição de refresh por instância do cliente HTTP, liberada após sucesso ou falha. Isso evita disputar a rotação do mesmo token dentro da página; não coordena abas diferentes. Sem sessão válida, `/auth/me` e `/auth/sessions/refresh` continuam retornando 401 e a SPA apresenta o login. O aviso de React DevTools é informativo.
