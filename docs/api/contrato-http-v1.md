...
| Campo                      | Valor                                                                                                                                                                                   |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status                     | Implementado no código-fonte e coberto por testes unitários/contratuais; a configuração versionada continua desabilitada por padrão.                                                    |
| Base verificada            | `https://api-formulario.rodogarcia.com.br/api/v1` — `GET /auth/csrf` respondeu `200` na verificação anônima de 2026-08-26; não houve teste autenticado fim a fim nem aceite de negócio. |
| Pré-requisitos para novo alvo | Histórico completo da fonte até `V0011`, identidade SQL de mínimo privilégio, configuração externa protegida, bootstrap controlado de administradores e validação autorizada por alvo.   |
...
Em 2026-08-26, `AVALIACAO_DEV` e `AVALIACAO_PROD` reconciliaram `V0001`–`V0010`; o catálogo inicial foi confirmado por leituras autorizadas e não havia ciclos, lotações, vínculos, atribuições ou avaliações. A `V0011`, presente na fonte, restringe a autoridade de Administrador e normaliza contas administrativas legadas para perfil único, mas ainda não foi aplicada a esses alvos. A verificação anônima dos hosts retornou `200` e os processos PM2 estavam online. A configuração padrão ainda não registra as rotas persistidas; o launcher local as ativa somente em processo de desenvolvimento controlado. Portanto, esse estado não equivale a aceite de negócio, a validação autenticada de ponta a ponta ou autorização para criar conta, migrar outro banco, alterar produção ou publicar nova versão. A sequência segura está em [Configuração externa da aplicação](../operations/configuracao-externa-da-aplicacao.md).

Até a aplicação autorizada da `V0011`, contas administrativas legadas dos alvos canônicos podem ainda refletir as concessões históricas. Elas não são evidência de que o contrato de perfil único já esteja ativo em produção.
