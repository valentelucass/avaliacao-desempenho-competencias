...
| Operação                                           | Regra                                                                                                                                   |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /assessments?limit=1..100&cursor=<opaco>`     | Retorna `{ items, page: { limit, nextCursor } }` no escopo do ator. `nextCursor` só deve ser reutilizado para a página seguinte.        |
...
Detalhes devolvem `id`, `cycle`, `evaluated`, `type`, `status`, `revision`, `updatedAt`, questionário, respostas e, quando aplicável, `result.finalScore` e `result.classification { label, guidance }`. Cada opção de resposta do questionário traz também seu `points` somente para exibição transparente ao avaliador; o cliente continua enviando exclusivamente `optionId` e o servidor continua sendo a única autoridade para validar a escala, calcular a nota e classificar o resultado. Rascunho não possui resultado. A nota é calculada no servidor, pertence a 80–120 e usa a regra `2024.1`.
