...
O banco local dedicado está reconciliado de `V0001` a `V0007`, incluindo a regra operacional e o catálogo RBAC. Os valores padrão seguem sem registrar módulos persistidos; `iniciar-dev.bat` os ativa apenas no processo de desenvolvimento local controlado. Antes de aceitar dados reais ou liberar outro ambiente, é obrigatório seguir [Configuração externa da aplicação](../operations/configuracao-externa-da-aplicacao.md), testar contra SQL Server autorizado e liberar a configuração por ambiente. A existência do código e do launcher local não equivale a uma API publicada ou a um sistema operacional em produção.
